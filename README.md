...
- Para a criação de stickers a partir de vídeos/gifs é necessário `ffmpeg` disponível no sistema. Você pode instalar via gerenciador de pacotes (`sudo apt install ffmpeg`) ou usar a dependência [`ffmpeg-static`](https://www.npmjs.com/package/ffmpeg-static) (`npm install ffmpeg-static`). O handler tenta localizar o binário automaticamente e informa caso não consiga.
